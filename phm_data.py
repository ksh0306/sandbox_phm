"""3상 모터 PHM 데이터 처리 파이프라인 (클래스 기반).

설계 의도
---------
PR 리뷰 피드백 ("FFT 변환 로직이 다른 코드와 섞여 재사용이 어렵다") 을 반영해
역할별로 클래스를 분리한다. 각 클래스는 자신의 데이터·메타데이터를 들고 있고
h5 파일로 직렬화할 수 있다. 새로운 변환을 추가하기 쉽도록 모든 변환은
입력 컨테이너를 받아 새 컨테이너를 반환하는 형태 (정적 메서드 / 모듈 함수) 로
구성되어 있다.

  H5DataContainer  — 베이스. h5 IO + 메타데이터 관리. 저장 파일명 지정 가능.
  ├── RawData       — (n_ch, N) 시간영역 신호 컨테이너 (raw / scaled / …).
  ├── FFTData       — (n_ch, n_win, n_bins) 단측 진폭 스펙트럼 컨테이너.
  ├── XcorrTimeData — 시간영역 9쌍 cross-correlation 결과.
  └── XcorrFFTData  — FFT 스펙트럼 9쌍 cross-correlation 결과.

전처리는 컨테이너 → 컨테이너의 자유로운 함수로 추가한다 (`log16p`, …).
무거운 변환은 각 결과 클래스의 정적 메서드로 둔다
(`FFTData.from_raw`, `XcorrTimeData.from_signal`, `XcorrFFTData.from_fft`).
"""
...
class H5DataContainer:
    """h5 파일 IO 와 메타데이터 관리를 담당하는 베이스 컨테이너.

    내부 상태는 두 가지뿐이다.
      * ``datasets``  : ``{이름: ndarray}`` — h5 dataset 으로 저장될 배열들.
      * ``attrs``     : ``{이름: 값}``      — h5 attribute (메타데이터).

    파생 클래스는 자신만의 access property 를 추가하되, h5 직렬화 자체는
    이 베이스의 ``save`` / ``load`` 를 그대로 재사용한다.
    ...
    attrs : dict[str, Any], optional
        h5 attribute 로 보존할 메타데이터 사전.
    default_filename : str | Path, optional
        ``save()`` 호출 시 경로를 생략하면 사용되는 기본 저장 파일명.
    """
    ...
    def __init__(self, datasets=None, attrs=None, default_filename=None):
        ...
        self.default_filename = (
            str(default_filename) if default_filename is not None else None
        )
    ...
    def save(self, out_path=None):
        """모든 dataset 과 attribute 를 h5 파일에 직렬화한다.

        out_path 가 ``None`` 이면 인스턴스의 ``default_filename`` 을 사용한다.
        부모 디렉토리는 필요시 자동 생성한다.
        """
        if out_path is None:
        ...
        with h5py.File(out_path, "w") as f:
            ...
            for name, arr in self.datasets.items():
                arr = np.asarray(arr)
                # 1D 이상의 의미있는 배열만 gzip 압축. scalar 는 그대로.
    ...
    def load(cls, in_path):
        """h5 파일에서 dataset 과 attribute 를 모두 읽어 인스턴스를 생성한다.

        ``cls`` 가 베이스라면 모든 데이터를 평평한 ``datasets`` 사전에 담은
        ``H5DataContainer`` 가 돌아온다. 파생 클래스가 ``load`` 를 override
        하지 않아도, 베이스의 사전 기반 동작 + 파생 클래스의 property 를
        통해 그대로 활용할 수 있다.
        """
...
class RawData(H5DataContainer):
    """6채널 시간영역 신호 + 부수 메타데이터 컨테이너.

    저장 형식
    --------
    /data             : (n_ch, N) 배열  ← 채널별 시간영역 신호 (raw chunk 의
                        ``fast_adc`` 를 (6, N) 으로 reshape 한 결과)
    /events           : raw chunk 들의 events 를 이어 붙인 배열 (있으면)
    /fast_flags       : (N_pkt,) 패킷별 플래그 (있으면)
    /fast_motor_state : (N_pkt,) 패킷별 모터 상태 (있으면)
    /lifetime         : (n_chunks,) 칩 lifetime 통계 (있으면)
    /motor_spec       : (n_chunks,) 모터 사양 (있으면)
    /slow_ctx         : (N_slow,) slow-context 데이터 (있으면)
    /ts_us            : (N_pkt,) 패킷별 타임스탬프 [μs] (있으면)
    attrs:
      fs_hz     : 샘플링 주파수 [Hz]
      channels  : 채널 이름 목록 (S 타입 array)
      source    : 전처리 단계 표시 (예: "raw", "scaled_log16p")
      그 외 raw chunk 의 top-level attrs (motor_id, fw_ver, pole_pairs, …) 가
      그대로 보존된다.
    """
    ...
    def __init__(self, data=None, fs_hz=FS_HZ, source=None, channels=None,
                 datasets=None, attrs=None, default_filename=None):
...
def log16p(raw_data: RawData) -> RawData:
    """LOG16P 정규화: ``log2(x + 1) / 16``.

    raw uint16 (0 ~ 65535) 을 약 [0, 1] 범위로 압축한다. FFT / xcorr 단계에서
    수치 안정성을 얻기 위한 표준 전처리.

    원본 RawData 의 부수 메타데이터 (events, fast_flags, lifetime, …) 와
    top-level attrs 는 그대로 보존하고 ``data`` 만 바뀐다.
    """
...
class FFTData(H5DataContainer):
    ...
    def __init__(self, fft_mag=None, freqs=None, fs_hz=FS_HZ,
                 samples_per_window=None,
                 window=b"hann", normalization=b"amplitude_single_sided",
                 source=b"scaled", channels=None,
                 datasets=None, attrs=None, default_filename=None):
        ds = {} if datasets is None else dict(datasets)
        at = {} if attrs is None else dict(attrs)
    ...
    @staticmethod
    def from_raw(raw_data: RawData,
                 samples_per_window: int = FFT_WINDOW_SAMPLES) -> "FFTData":
        ...
        * "동일 진폭의 정현파라면 막대 높이가 그 진폭에 가깝다" 가 성립하도록
        ...
        extra_attrs = {k: v for k, v in raw_data.attrs.items()
                       if k not in {"source", "channels"}}
        return FFTData(
            fft_mag=out, freqs=freqs, fs_hz=fs,
    ...
    def inverse_fft(fft_data: "FFTData") -> np.ndarray:
        ...
        for i in range(n_chan):
            ...
            if spw % 2 == 0:
                mag[:, -1] *= 2.0
            X = mag / 2.0  # *2 / Σwin 의 역 — 단, Σwin 은 IFFT 후 보정 불필요(상대형상만 필요).
            for w in range(n_win):
...
# ─────────────────────────────────────────────────────────────────────────────
def _normalized_xcorr_pair(xa: np.ndarray, xb: np.ndarray,
...
class XcorrTimeData(H5DataContainer):
    ...
    def __init__(self, xcorr_dict=None, lags=None, samples_per_window=None,
                 max_lag=None, source=b"scaled", fs_hz=FS_HZ,
                 datasets=None, attrs=None, default_filename=None):
        ds = {} if datasets is None else dict(datasets)
        at = {} if attrs is None else dict(attrs)
        if xcorr_dict is not None:
            ...
            ds["xcorr"] = np.stack([np.asarray(xcorr_dict[n]) for n in pair_names],
                                   axis=0)
    ...
    @staticmethod
    def from_signal(raw_data: RawData,
        ...
        return XcorrTimeData(
            xcorr_dict=result, lags=lags,
            samples_per_window=samples_per_window,
            max_lag=max_lag, fs_hz=raw_data.fs_hz,
            source=raw_data.source,
            datasets=extra_datasets,
            attrs=extra_attrs,
        )
...
class XcorrFFTData(H5DataContainer):
    ...
    def __init__(self, xcorr_dict=None, lags=None, samples_per_window=None,
                 max_lag=None, fs_hz=FS_HZ, source=b"scaled",
                 datasets=None, attrs=None, default_filename=None):
        ds = {} if datasets is None else dict(datasets)
        at = {} if attrs is None else dict(attrs)
        if xcorr_dict is not None:
    ...
    def from_fft(fft_data: FFTData,
        ...
        for a, b in pairs:
            ...
            for w in range(n_win):
                out[w] = _normalized_xcorr_pair(
                    fft_mag[ia, w], fft_mag[ib, w], max_lag,
                )
            result[f"{a}-{b}"] = out

        # fft_data 의 부수 데이터셋을 통과 — 단 FFTData 의 스펙트럼/주파수
        # 데이터셋은 XcorrFFTData 와 의미가 다르므로 제외한다.
        extra_datasets = {k: v for k, v in fft_data.datasets.items()
                          if k not in {"data", "freqs"}}
        exclude_attrs = {"source", "channels", "window", "normalization",
                         "samples_per_window"}
        extra_attrs = {k: v for k, v in fft_data.attrs.items()
                       if k not in exclude_attrs}
        return XcorrFFTData(
            xcorr_dict=result, lags=lags,
            samples_per_window=fft_data.samples_per_window,
            max_lag=max_lag, fs_hz=fft_data.fs_hz,
